...
class _MainScreenState extends State<MainScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: PageView(
        ...
        children: <Widget>[//modiiiiiiiiiiiiiiiiifs
  ...
  title: Row(
    ...
    children: [
      Padding(
        padding: EdgeInsets.only(left: 30.0), // Décaler "Bienvenue !" de 30 pixels sur la droite
        child: const Text(
          'Bienvenue !',
      ...
      Padding(
        padding: EdgeInsets.only(top: 50.0), // Décaler l'astronaute de 30 pixels vers le bas
        child: Align(
          alignment: Alignment.topRight,
          child: SvgPicture.asset(
            'assets/SVG/astronaut.svg',
            height: 110.0, // La hauteur peut être ajustée si nécessaire
            semanticsLabel: 'Astronaute',
          ),
        ),
      ),
    ],
 ...
 Widget _buildSectionTitle(String title) {
  return Padding(
    ...
    child: Row(
      ...
      children: [//début code ajouté
        Row(
          mainAxisSize: MainAxisSize.min, // Pour que la rangée prenne la taille de son contenu
          children: [
            // Le petit cercle orange
            Container(
              width: 10.0,
              height: 10.0,
...
ElevatedButton(
  onPressed: () {
      ...
      if(title=="Séries populaires")
      {
        _onItemTapped(2);
      }
      if(title=="Films populaires")
      {
        _onItemTapped(3);
      }
  },
  child: const Text(
    'Voir plus',
    style: TextStyle(
      color: Colors.white, // Adapte la couleur du texte à ton thème
    ),
  ),
  style: ElevatedButton.styleFrom(
    backgroundColor: Colors.black.withOpacity(0.5), // Couleur de fond plus sombre avec une transparence
    shape: RoundedRectangleBorder(
      borderRadius: BorderRadius.circular(20), // Bords arrondis
    ),
    padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8), // Ajuste le padding si nécessaire
  ),
)
